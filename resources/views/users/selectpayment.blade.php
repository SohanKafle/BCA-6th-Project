                ...
                @foreach($selectpayments as $payment)
                    @if($payment->car_id == $car->id) <!-- Ensure this is the current car -->
                        <div class="flex items-center mt-4">
                            <img src="{{ asset('uploads/cars/' .$payment->photopath) }}" alt="Car Image" class="w-32 h-32 object-cover rounded-lg mr-4"> <!-- Larger Image -->
                            <div>
                                <p class="text-lg font-semibold">{{ $car->name }}</p> <!-- Display current car name -->
                                <p class="text-lg">Price: <span class="text-green-500 font-semibold">Rs {{ number_format($car->price) }}</span></p> <!-- Display current room price -->
                            </div>
                        </div>
                    @endif
                @endforeach
